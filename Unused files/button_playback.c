...
//volatile uint8_t start_recording = 0; // 0 = did not just start recording, 1 = just started recording
volatile uint8_t recording = 0; // 0 = not recording, 1 = in the process of recording
//volatile uint8_t playback = 0; // 0 = in the process of recording, 1 = not recording
//volatile uint8_t start_playback = 0; // 0 = did not just start playback, 1 = just started playback

void init_buttons(void) {
//    printf("entered init buttons\n");
    // Set PD2, PD3, PD4 as input
    DDRD &= ~(1 << PD2);
    PORTD |= (1 << PD2);  // Enable pull-ups
...
// === INT0 (PD2)
ISR(INT0_vect) {
//    printf("entered button isr\n");
    if (int0_waiting_rising) {
//        start_recording ^= 1; // toggle recording flag
        recording ^= 1; // toggle recording flag
//        printf("recording: %d\n", recording);
    }
 ...
 volatile Note melody[MAX_NOTE_COUNT]; // 300 * 4 = 1200 bytes
 volatile int melody_idx = 0; // number of notes in the melody
 
 
 // start time array (separate from struct) to account for overlapping notes (mel)
 #define MAX_MIDI_NOTES 128
volatile uint16_t note_start_times[MAX_MIDI_NOTES] = {0};  // 0 means not active

 
 volatile uint32_t tic = 0; // Timer counter for 10ms intervals
 volatile uint16_t note_start_time = 0; // Time when the note started (in ms)
...
//        if (recording) {
            ...
            tic = 0;
            clear_melody(melody);  // Optional: clear previous melody if recording again

            while (recording) {  // stay in this loop until button toggles it off
...
//                    _delay_ms(100);
                    ...
                    if (current_note == data1) stop_note();
...
//        } 
//        else {
//            printf("Recording stopped. Playing...\n");
////            _delay_ms(100);
//            play_melody(melody);
//        }
//    }
...
//     if (recording == 1) {
//         printf("recording started\n");
//         tic = 0; // reset tic for starting the recording
//        while(1) {
//            status = uart_receive(NULL);
//            data1 = uart_receive(NULL);
//            data2 = uart_receive(NULL);
